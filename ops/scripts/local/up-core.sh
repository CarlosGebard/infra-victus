...
write_managed_env_block() {
  ...
  awk '
    ...
    $1 ~ /^(TAILSCALE_INTERFACE|TAILSCALE_IPV4|DNS_ZONE|ETCD_PREFIX|DNS_TTL|COREDNS_BIND_IP|COREDNS_DNS_PORT|NGINX_BIND_IP|NGINX_HTTP_PORT|S3_DOMAIN|POSTGRES_DOMAIN|POSTGRES_BIND_IP|POSTGRES_PORT|REDIS_DOMAIN|REDIS_BIND_IP|REDIS_PORT|LITELLM_DOMAIN|LANGFUSE_DOMAIN)=/ { next }
    skip == 0 { print }
  ' "$ENV_FILE" > "$tmp_file"
...
LITELLM_DOMAIN=${LITELLM_DOMAIN:-litellm.${DNS_ZONE:-victus.io}}
LANGFUSE_DOMAIN=${LANGFUSE_DOMAIN:-langfuse.${DNS_ZONE:-victus.io}}
